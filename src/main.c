...
static void my_task_func(struct worker_thread_timer_task_s* task) {
    ...
    if (millis()-char_recv_ms > 100 && scanned_code_len > 0) {
        struct com_matternet_equipment_scanner_BarcodeContent_s barcode_msg;
        barcode_msg.content_string_len = scanned_code_len > sizeof(barcode_msg.content_string) ? sizeof(barcode_msg.content_string) : scanned_code_len;
        memcpy(barcode_msg.content_string, scanned_code, barcode_msg.content_string_len);
        scanned_code_len = 0;
        chSysUnlock();
